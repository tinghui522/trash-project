...
function pipeOutputToLogger({ stdout, stderr } = {
    ...
    if (stdout) {
        stdout.on('data', chunk => logMultiline(chunk, { ...extraFields, source: 'stdout' }, loggerLineTransformer));
    }
    if (stderr) {
        const source = stdoutOnly ? 'stdout' : 'stderr';
        stderr.on('data', chunk => logMultiline(chunk, { ...extraFields, source }, loggerLineTransformer));
    }
}
