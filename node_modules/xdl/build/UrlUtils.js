...
async function constructDevClientUrlAsync(projectRoot, opts, requestHostname) {
    ...
    const manifestUrl = await constructManifestUrlAsync(projectRoot, { ...opts, urlType: 'http' }, requestHostname);
...
function resolveProtocol(projectRoot, { urlType, ...options }) {
    if (urlType === 'http') {
        return 'http';
    }
...
async function constructUrlAsync(projectRoot, incomingOpts, isPackager, requestHostname) {
    ...
    if (proxyURL) {
        ...
        if (!ngrokUrl || typeof ngrokUrl !== 'string') {
            ...
            return constructUrlAsync(projectRoot, { ...opts, hostType: 'lan' }, isPackager, requestHostname);
        }
        else {
