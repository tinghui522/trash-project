...
exports.getCacheDir = exports.Cacher = void 0;
...
class Cacher {
    constructor(refresher, filename, ttlMilliseconds, bootstrapFile) {
        ...
        this.filename = path_1.default.join(getCacheDir(), filename);
    ...
    async getAsync() {
        ...
        let mtime;
        try {
            const stats = await fs_extra_1.default.stat(this.filename);
            mtime = stats.mtime;
        }
        catch (e) {
            try {
                await fs_extra_1.default.mkdirp(getCacheDir());
                if (this.bootstrapFile) {
                    const bootstrapContents = (await fs_extra_1.default.readFile(this.bootstrapFile)).toString();
                    await fs_extra_1.default.writeFile(this.filename, bootstrapContents, 'utf8');
                }
            }
            catch (e) {
                // intentional no-op
        ...
        if (this.ttlMilliseconds === 0 ||
            ...
            try {
                ...
                try {
                    await fs_extra_1.default.writeFile(this.filename, JSON.stringify(fromCache), 'utf8');
                }
                catch (e) {
                    this.writeError = e;
        ...
        if (!fromCache) {
            try {
                fromCache = JSON.parse(await fs_extra_1.default.readFile(this.filename, 'utf8'));
            }
            catch (e) {
                this.readError = e;
    ...
    async clearAsync() {
        try {
            await fs_extra_1.default.unlink(this.filename);
        }
        catch (e) {
            this.writeError = e;
        }
...
function getCacheDir() {
    ...
    if (process.env.XDG_CACHE_HOME) {
        ...
        return path_1.default.join(homeDir, 'AppData', 'Local', 'Expo');
    }
    else if (process.platform === 'darwin') {
        // too many mac users have broken permissions on their ~/.cache directory
        return path_1.default.join(homeDir, '.expo', 'cache');
    }
    else {
        return path_1.default.join(homeDir, '.cache', 'expo');
    }
}
