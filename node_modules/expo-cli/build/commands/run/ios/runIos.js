...
async function runIosActionAsync(projectRoot, options) {
  ...
  if (props.isSimulator) {
    _ExpoLogFormatter().ExpoLogFormatter.logPrettyItem(`${_chalk().default.bold`Installing`} on ${props.device.name}`);
...
async function openInSimulatorAsync({
  ...
  _ExpoLogFormatter().ExpoLogFormatter.logPrettyItem(`${_chalk().default.bold`Opening`} on ${device.name} ${_chalk().default.dim(`(${bundleIdentifier})`)}`);
